...
public class CarDaoImpl extends JdbcDaoSupport implements CarDao {
    ...
    @Override
    public List<Car1> getCarByCarId(LocalDate start, LocalDate end) {

        System.out.println(start.isBefore(end));
        System.out.println(start);
        LocalDate a = start;
        String sql = "select * from car where Start_Time>=? and End_Time<=? and status=1";
    ...
    {
        String resSQL = "select * from car where token=?";
